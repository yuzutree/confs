...
vec4 hook() {
    ...
    for (float k = 0.0; k < high - low; k++) {
        ...
        avg += w * pow(textureLod(PREKERNEL_raw, pos, 0.0) - textureLod(POSTKERNEL_raw, POSTKERNEL_pos, 0.0), vec4(2.0));
        W += w;
    }
    avg /= W;
...
mat2x4 ScaleH(vec2 pos, vec4 mean) {
    // Calculate bounds
    float low  = floor(-0.5*maxtaps - offset)[0];
    float high = floor(+0.5*maxtaps - offset)[0];
    ...
    for (float k = 0.0; k < maxtaps; k++) {
        ...
        avg += w * mat2x4(pow(textureLod(POSTKERNEL_raw, pos, 0.0) - mean, vec4(2.0)), textureLod(Var_raw, pos, 0.0));
        W += w;
    }
    avg /= W;
